...
export default async function handler(req, res) {
  ...
  try {
    ...
    const eventDataLength = JSON.stringify(eventData).length;
    if (eventDataLength > 4096) {
      return res.status(400).json({ error: 'eventData is too large' });
    }
